...
for process in "${processes[@]}"; do
	...
	mkdir -p $output_dir
	python jobSubmission/create-condor-jobs -i production/inputFiles_$process.txt -o $output_dir/out.root -c config/cmssw_centralMC_Tag_B_MuDst-PiPiK.py --maxtime 30m -N 5 -f
	sleep 1
    fi
done
